...
public class Bry_rdr {
	...
	public void Chk_trie_val(Btrie_slim_mgr trie, byte val) {
		...
		if (rv == Byte_.Max_value_127) 
                    err_wkr.Fail("failed trie check", "mid", String_.new_u8(Bry_.Mid_by_len_safe(src, pos, 16)));
	}
	public Object Chk_trie_as_obj(Btrie_rv trv, Btrie_slim_mgr trie) {
		Object rv = trie.Match_at(trv, src, pos, src_end); 
                if (rv == null) 
                    err_wkr.Fail("failed trie check", "mid", String_.new_u8(Bry_.Mid_by_len_safe(src, pos, 16)));
		return rv;
	}
	...
	public byte Chk(Btrie_slim_mgr trie, int itm_bgn, int itm_end) {
		...
		if (rv == Byte_.Max_value_127) {
                    err_wkr.Fail("failed trie check", "mid", String_.new_u8(Bry_.Mid_by_len_safe(src, pos, 16))); return Byte_.Max_value_127;}
		return rv;
	}

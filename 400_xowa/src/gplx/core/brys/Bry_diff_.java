	...
	public static byte[][] Diff_1st(byte[] lhs, int lhs_bgn, int lhs_end, byte[] rhs, int rhs_bgn, int rhs_end, byte[] stop, byte[] show, int diff_max) {
		...
		for (int i = 0; (i < lhs_len) && (i < rhs_len); ++i) {
			...
			if (lhs_byte != rhs_byte) {
				// skip lhs whitspace
				while (i < lhs_len && (lhs_byte == Byte_ascii.Space || lhs_byte == Byte_ascii.Nl)) {
					lhs_bgn++;
                                        lhs_len--;
					lhs_byte = lhs[i + lhs_bgn];
				}
				// skip rhs whitespace
				while (i + rhs_bgn < rhs_len && (rhs_byte == Byte_ascii.Space || rhs_byte == Byte_ascii.Nl)) {
					rhs_bgn++;
                                        rhs_len--;
					rhs_byte = rhs[i + rhs_bgn];
				}
				if (lhs_byte == Byte_ascii.Slash && rhs_byte == Byte_ascii.Gt && lhs[i + lhs_bgn + 1] == Byte_ascii.Gt) {
					lhs_bgn++;
                                        lhs_len--;
					lhs_byte = lhs[i + lhs_bgn];
				}

...
//	public boolean Has(String key) {return list.Has(key);}
	...
	public void HxtnSave(Xowe_wiki wiki, Hxtn_page_mgr hxtnPageMgr, Xoae_page page, int pageId) {
		...
		byte[] crumb = RelatedSerialCore.Save(isin);
		hxtnPageMgr.Page_tbl__insert(pageId, Hxtn_page_mgr.Id__related, pageId);
		hxtnPageMgr.Blob_tbl__insert(Hxtn_blob_tbl.Blob_tid__wtxt, Hxtn_page_mgr.Id__related, pageId, crumb);
	}
	...
	public void Deserialise(Xow_wiki wiki, Xoh_page hpg, byte[] data) {
		...
		// deserialize data
		this.isin = RelatedSerialCore.Load(data);
	}
}

...
public class Ref_html_wtr_cfg {
	...
	public void Init_by_wiki(Xowe_wiki wiki) {
		...
		/*byte[] ref_bry = wiki.Msg_mgr().Val_by_key_obj(Msg_reflink);
                if (ref_bry != null)
                    Itm_html_(ref_bry.toString());
		ref_bry = wiki.Msg_mgr().Val_by_key_obj(Msg_reflinkone);
                if (ref_bry != null)
                    Grp_html_one_(ref_bry.toString());
		ref_bry = wiki.Msg_mgr().Val_by_key_obj(Msg_reflinkmany);
		ref_bry = wiki.Msg_mgr().Val_by_key_obj(Msg_reflinkmanyformat);*/
	}
	public byte[] Get_itm(Ref_nde itm, boolean caller_is_ref) {
		itm_tmp_bfr.Clear();
		itm_tmp_bfr.Add(Cite_ref);
	...
	public byte[] Get_grp(Ref_nde itm) {
        ...
        public void Build_ref(Ref_nde itm, boolean caller_is_ref, byte[] txt) {
                ...
                byte[] ref = wiki.Msg_mgr().Val_by_key_args(Msg_reflink, bitm, bgrp, txt);
        }
	...
	public static final    byte[] Msg_backlabels_err = Bry_.new_a7("cite_error_no_link_label_group")
                ...
                , Msg_reflink = Bry_.new_a7("Cite_reference_link")
                , Msg_reflinkmany = Bry_.new_a7("Cite_references_link_many")
                , Msg_reflinkmanyformat = Bry_.new_a7("Cite_references_link_many_format")
                , Msg_reflinkone = Bry_.new_a7("Cite_references_link_one")
                , Cite_ref = Bry_.new_a7("cite_ref-")
                , Cite_note = Bry_.new_a7("cite_note-")
                ;
	...
	public static Ref_html_wtr_cfg new_() {
		...
		//rv.Grp_html_one_	("<li id=\"cite_note-~{grp_id}\"><span class=\"mw-cite-backlink\">'''[[#~{1}|^]]'''</span> <span class=\"reference-text\">~{text}</span></li>\n");
		...
		rv.Grp_html_list_	(" <sup><i><b><a href=\"#cite_ref-~{itm_id}\">~{backlabel}</a></b></i></sup>");
		rv.Grp_id_uid_		("~{uid}");

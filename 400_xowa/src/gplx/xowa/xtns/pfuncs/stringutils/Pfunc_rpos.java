...
public class Pfunc_rpos extends Pf_func_base {
	...
	@Override public void Func_evaluate(Bry_bfr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {
		...
		if (offset_bry == null)
			...
			offset = Bry_.To_int_or_neg1(offset_bry);
			if (offset > 0)
				offset = gplx.core.intls.Utf8_.Count(argx, 0, offset); // from character count to byte count
		}
		int pos = Bry_find_.Find_bwd(argx, find, offset);
		if (pos == Bry_find_.Not_found)
			pos = -1;
		else
			pos = gplx.core.intls.Utf8_.Len_of_bry(argx, 0, pos); // from byte count to character count

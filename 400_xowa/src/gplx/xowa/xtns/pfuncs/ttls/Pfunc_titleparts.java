...
public class Pfunc_titleparts extends Pf_func_base {
	...
	// max of 25 parts (see https://www.mediawiki.org/wiki/Help:Extension:ParserFunctions
	// max length 255
	// one or two dots return original string
	@Override public void Func_evaluate(Bry_bfr bfr, Xop_ctx ctx, Xot_invk caller, Xot_invk self, byte[] src) {// REF.MW:ParserFunctions_body.php
		...
		if (argx_as_ttl == null || argx.length > 255) {
			bfr.Add(argx);
			return;	// NOTE: argx_as_ttl will be null if invalid, such as [[a|b]]; PAGE:en.w:owl and {{taxobox/showtaxon|Dinosauria}}
		}
		// get parts_len
	...
	private byte[] TitleParts(byte[] src, int parts_len, int parts_bgn, byte[] ttl) {
		...
		synchronized (dlms_ary) {// LOCK:static-obj; DATE:2016-07-06
			...
			boolean dotty = true;
			int dotcount = 0;
			for (int i = 0; i < src_len && dlms_ary_len < 25; i++) {
				byte b = src[i];
				switch(b) {

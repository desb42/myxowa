...
public class Xop_lnki_wkr implements Xop_ctx_wkr, Xop_arg_wkr {
	...
	public int Make_tkn(Xop_ctx ctx, Xop_tkn_mkr tkn_mkr, Xop_root_tkn root, byte[] src, int src_len, int bgn_pos, int cur_pos) {
		...
		if (lnki.Ns_id() != Xow_ns_.Tid__main && Bry_.Len_eq_0(lnki.Ttl().Page_txt()))	// handle anchor-only pages; EX:[[File:#A]] PAGE:en.w:Spindale,_North_Carolina; DATE:2015-12-28
			return Xop_lnki_wkr_.Invalidate_lnki(ctx, src, root, lnki, bgn_pos);
/* temporarily remove 20211008 
   "[[a|[b]]]" should be handled by line 38
   "[a [[b]]]" should only consume the 2 close quotes
...
//                                boolean xe = arg.KeyTkn_exists();int xbgn = arg.Src_bgn(), xend = arg.Src_end();
				switch (arg_tid) {
					...
					case Xop_lnki_arg_parser.Tid_thumb:
						if (arg.KeyTkn_exists()) {
							lnki.Thumb_alt_(arg); 
							lnki.Thumb_alt().Tkn_ini_pos(false, arg.Src_bgn(), arg.Src_end());
//						byte[] fred=lnki.Thumb_alt().v.Val_tkn();
						}
						lnki.Lnki_type_(Xop_lnki_type.Id_thumb);
						break;

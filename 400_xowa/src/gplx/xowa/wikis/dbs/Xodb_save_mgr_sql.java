...
public class Xodb_save_mgr_sql implements Xodb_save_mgr {
	...
	public int Data_create(Xowe_wiki wiki, Xoa_ttl ttl, byte[] text_raw) {
		...
		try {
			db_mgr.Core_data_mgr().Create_page(page_core_tbl, page_text_db, page_id, ns_id, ttl.Page_db(), redirect, Datetime_now.Get(), text_zip, text_raw.length, ns_count, 0);
	...
	public void Data_update(Xoae_page page, byte[] text_raw) {			
		...
		db_mgr.Core_data_mgr().Tbl__page().Update__redirect__modified(page_id, redirect, modified, text_raw.length);

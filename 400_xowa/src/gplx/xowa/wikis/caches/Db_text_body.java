...
public class Db_text_body {
	...
	public Db_text_body(Xoa_app app, String domain, long maxsize) {
		file_count = 0;
		this.app = app;
		this.domain = domain;
		Generate_next_file();
	...
	private void Generate_next_file() {
		...
		Io_url filename = app.Fsys_mgr().Wiki_dir().GenSubFil_nest(domain, domain + "-text-" + Integer.toString(++file_count) + ".dat");
	...
	private long Write(long page_id, byte[] v) {

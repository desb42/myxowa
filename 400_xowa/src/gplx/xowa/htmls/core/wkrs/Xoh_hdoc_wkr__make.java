...
public class Xoh_hdoc_wkr__make implements Xoh_hdoc_wkr {
	...
	private Xoh_page_bfr page_bfr = null;
	private boolean toc_enabled;
	private int html_uid;
	public Bry_bfr Bfr() {return bfr;}
	@Override public void On_page_bgn(Bry_bfr bfr, Xoh_page hpg, Xoh_hdoc_ctx hctx, byte[] src, int src_bgn, int src_end) {
            bfr.SetThreadId();
		this.bfr = bfr; this.hpg = hpg; this.hctx = hctx; this.src = src;
		this.html_uid = 0;
		this.toc_enabled = !gplx.core.envs.Op_sys.Cur().Tid_is_drd();
		if (toc_enabled)
			this.page_bfr = new Xoh_page_bfr(bfr);
	}
	@Override public void On_page_end() {
		if (toc_enabled)
			page_bfr.Commit(hpg);
	}
	@Override public void On_txt(int rng_bgn, int rng_end) {
		// text; just add it
		bfr.Add_mid(src, rng_bgn, rng_end);
	}
	@Override public void On_escape(gplx.xowa.htmls.core.wkrs.escapes.Xoh_escape_data data) {
		// hzip escape byte ((byte)27); should never happen but if it does, add it
		bfr.Add(data.Hook());
	}
	@Override public void On_xnde(gplx.xowa.htmls.core.wkrs.xndes.Xoh_xnde_parser data) {
		// regular xml node; just add it
		bfr.Add_mid(src, data.Src_bgn(), data.Src_end());
	}
	@Override public void On_lnki(gplx.xowa.htmls.core.wkrs.lnkis.Xoh_lnki_data data)	{
		// <a> node
	...
	@Override public boolean On_thm(gplx.xowa.htmls.core.wkrs.thms.Xoh_thm_data data) {
	...
	@Override public void On_gly		(gplx.xowa.htmls.core.wkrs.glys.Xoh_gly_grp_data data) {
	...
	@Override public boolean Process_parse(Xoh_data_itm data) {
            bfr.SetThreadId(); // ugh - this should not be done here!
		switch (data.Tid()) {

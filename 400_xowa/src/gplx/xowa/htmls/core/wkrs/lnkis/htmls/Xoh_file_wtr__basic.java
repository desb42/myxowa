		...
		if (lnki_is_thumbable) {	// is "thumb"
			...
			img_fmtr.Add_full_img(tmp_bfr, hctx, page, src, xfer_itm, uid, lnki_href, Bool_.N, Xoh_lnki_consts.Tid_a_cls_image, Xoh_lnki_consts.Tid_a_rel_none, anch_ttl
				, Xoh_file_fmtr__basic.Escape_xowa_title(lnki_ttl)
                                ...
                                , xfer_itm.Html_h()
                                , img_view_src, alt
//				, Xoh_file_fmtr__basic.Escape_xowa_title(lnki_ttl), xfer_itm.Lnki_w(), xfer_itm.Lnki_h(), img_view_src, alt
			...
			if (lnki_link_tkn == Arg_nde_tkn.Null)		// link absent; just write it
				...
				// only encode the path
				int upto = Bry_find_.Find_fwd(link_arg, Byte_ascii.Question);
				if (upto == -1) upto = Bry_find_.Find_fwd(link_arg, Byte_ascii.Hash);
				if (upto == -1)
					...
					byte[] path = Bry_.Mid(link_arg, 0, upto);
					path = Gfo_url_encoder_.Href_quotes_v2.Encode(path);
					link_arg = Bry_.Add(path, Bry_.Mid(link_arg, upto, link_arg.length));
				}

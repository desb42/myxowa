...
//wrong no of params		fxt.Wiki().Html_mgr().Portal_mgr().Div_view_fmtr().Fmt_("~{portal_view_read_href};~{portal_view_edit_href};~{portal_view_html_href}");
...
//wrong no of params		Tfds.Eq(expd, String_.new_a7(wiki.Html_mgr().Portal_mgr().Div_ns_bry(wiki.Utl__bfr_mkr(), Xoa_ttl.Parse(wiki, Bry_.new_a7(ttl)), wiki.Ns_mgr())));
	}
...
//wrong no of params		Tfds.Eq(expd, String_.new_a7(wiki.Html_mgr().Portal_mgr().Div_personal_bry(false)));
	}
	public void Test__div_view(byte output_tid, String ttl_str, String expd) {
		Xoa_ttl ttl = wiki.Ttl_parse(Bry_.new_u8(ttl_str));
//wrong no of params		Gftest.Eq__ary__lines(expd, String_.new_u8(wiki.Html_mgr().Portal_mgr().Div_view_bry(app.Utl__bfr_mkr(), output_tid, Bry_.Empty, ttl)));
	}
	public void Test__div_ns(String ttl_str, String expd) {
		Xoa_ttl ttl = wiki.Ttl_parse(Bry_.new_u8(ttl_str));
//wrong no of params		Gftest.Eq__ary__lines(expd, String_.new_u8(wiki.Html_mgr().Portal_mgr().Div_ns_bry(app.Utl__bfr_mkr(), ttl, wiki.Ns_mgr())));
	}
}

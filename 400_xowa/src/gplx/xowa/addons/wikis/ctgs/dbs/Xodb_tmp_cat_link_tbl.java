...
*/
package gplx.xowa.addons.wikis.ctgs.dbs; import gplx.*; import gplx.xowa.*; import gplx.xowa.addons.*; import gplx.xowa.addons.wikis.ctgs.*;
...
public class Xodb_tmp_cat_link_tbl implements Db_tbl {
	...
	public void Insert_cmd_by_batch(int page_id, byte[] ctg_ttl, byte[] sortkey, long timestamp, byte[] sortkey_prefix, byte collation_id, byte type_id) {
		stmt_insert.Clear()
			...
			.Val_int(fld_cat_subcats				, 0)
			.Val_int(fld_cat_pages				, 0)
			.Val_int(fld_cat_files				, 0)
			.Exec_insert();
	}

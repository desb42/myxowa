...
*/
package gplx.xowa.addons.wikis.ctgs.htmls.catpages.dbs; import gplx.*; import gplx.xowa.*; import gplx.xowa.addons.*; import gplx.xowa.addons.wikis.ctgs.*; import gplx.xowa.addons.wikis.ctgs.htmls.catpages.*;
...
class Xoctg_catlink_loader {
	...
	public void Run(Xoctg_catpage_ctg rv, int cat_id, byte grp_tid, boolean url_is_from, byte[] url_sortkey, int limit) {
		...
		grp.Itms_(wiki, (Xoctg_catpage_itm[])catlink_hash.To_ary_and_clear(Xoctg_catpage_itm.class), catpage_mgr.Collation_mgr());
	...
	private String Generate_sql(int cat_id, byte grp_tid, boolean url_is_from, byte[] url_sortkey, int limit, int link_db_id) {
		...
		), sortkey_col, sortkey_prefix_fld);
		
		if (grp_tid > 0) {
			bfr.Add_str_u8(String_.Concat_lines_nl
			( ",       0 AS cat_subcats"
			, ",       0 AS cat_pages"
			, ",       0 AS cat_files"));
		}
...
//			Xoctg_catpage_mgr.rwl.writeLock().lock();
	...
	private void Load_sortkey(Xoctg_catpage_grp grp, Xoctg_catpage_itm zth_itm) {
		...
		zth_itm.Sortkey_handle_make(Bry_bfr_.New(), wiki, prv_sortkey, catpage_mgr.Collation_mgr());
	}

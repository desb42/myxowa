...
*/
package gplx.xowa.addons.bldrs.mass_parses.parses.mgrs; import gplx.*; import gplx.xowa.*; import gplx.xowa.addons.*; import gplx.xowa.addons.bldrs.*; import gplx.xowa.addons.bldrs.mass_parses.*; import gplx.xowa.addons.bldrs.mass_parses.parses.*; import gplx.core.envs.*;
	...
	public void Mark_done(int id) {
		synchronized (thread_lock) {
			...
			if (pages_done % prog_interval == 0) {
				...
				Gfo_usr_dlg_.Instance.Prog_many("", "", "done=~{1} left=~{2} rate=~{3} time_past=~{4} time_left=~{5} mem_tot=~{6} mem_free=~{7}", 
							id, pages_done, pages_left, (int)rate_cur, time_past, time_left, Runtime_.Memory_total(), Runtime_.Memory_free());
				prog_prv = prog_cur;
			}

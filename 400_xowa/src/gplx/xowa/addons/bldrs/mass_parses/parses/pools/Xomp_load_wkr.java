...
*/
package gplx.xowa.addons.bldrs.mass_parses.parses.pools; import gplx.*; import gplx.xowa.*; import gplx.xowa.addons.*; import gplx.xowa.addons.bldrs.mass_parses.*; import gplx.xowa.addons.bldrs.mass_parses.parses.*;
import gplx.dbs.*; import gplx.core.threads.utils.*; import gplx.xowa.addons.bldrs.mass_parses.parses.utls.*; import gplx.core.envs.*;
	...
	public void Mark_done(int id) {
		synchronized (thread_lock) {
			...
			if (pages_done % 1000 == 0) {
				...
				Gfo_usr_dlg_.Instance.Prog_many("", "", "done=~{0} left=~{1} rate=~{2} time_past=~{3} time_left=~{4} mem_tot=~{5} mem_free=~{6}", 
							pages_done, pages_left, (int)rate_cur, time_past, time_left, Runtime_.Memory_total(), Runtime_.Memory_free());
				time_prv = time_cur;
			}
		}
	}

...
public class Db_minerva_skin implements Db_skin {
	...
	public byte[] Content(Xow_portal_mgr portal_mgr, byte[] page_data, Xoae_page page, Xowe_wiki wiki, Xop_ctx ctx, Xoh_wtr_ctx hctx, byte html_gen_tid, byte[] pagename_for_h1, byte[] modified_on_msg, int page_mode, Bry_bfr tmp_bfr, boolean ispage_in_wikisource, Wdata_xwiki_link_wtr wdata_lang_wtr, boolean ctgs_enabled, Xoh_page_wtr_mgr mgr) {
		...
		boolean is_legacy = false; // for vector skin
...
*/
		byte[] content = Pagebody(portal_mgr, page_data, page, page_mode, wiki, mgr);
		content = Bry_.Add(Bry_.new_a7("<div id=\"bodyContent\" class=\"content\">\n"), content, Bry_.new_a7("</div>\n"));
		data.AddKvStr("contenthtml", content);

//			'placeholder' => wfMessage( 'mobile-frontend-placeholder' )
		data.AddKvStr("placeholder", msgvalue("mobile-frontend-placeholder", wiki));
//			'main-menu-tooltip' => $this->getMsg( 'mobile-frontend-main-menu-button-tooltip' ),
		data.AddKvStr("main-menu-tooltip", msgvalue("mobile-frontend-main-menu-button-tooltip", wiki));
...
//			'userNavigationLabel' => wfMessage( 'minerva-user-navigation' ),
		data.AddKvStr("main-menu-tooltip", msgvalue("minerva-user-navigation", wiki));
/*
...
*/
		data.AddKvStr("searchButton", "<button id=\"searchIcon\" class=\"mw-ui-icon mw-ui-icon-element mw-ui-icon-wikimedia-search-base20 skin-minerva-search-trigger\" type=\"submit\">Search</button>");

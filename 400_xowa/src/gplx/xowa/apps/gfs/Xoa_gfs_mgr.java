...
public class Xoa_gfs_mgr implements Gfo_invk, Gfo_invk_root_wkr {
	...
	public void Run_url(Io_url url) { Run_url(url, false); }
	public void Run_url_for(Gfo_invk invk, Io_url url) { Run_url_for(invk, url, false); }
	public void Run_url(Io_url url, boolean showerror) {
		Run_url_for(GfsCore.Instance.Root(), url, showerror);
		Gfo_usr_dlg_.Instance.Log_wkr().Log_to_session_fmt("gfs.done: ~{0}", url.Raw());
	}
	public void Run_url_for(Gfo_invk invk, Io_url url, boolean showerror) {
		String raw;
		if (showerror)
			raw = Io_mgr.Instance.LoadFilStr_args(url).Exec();
		else

...
public class Xol_lang_itm implements Gfo_invk {
	...
	public Xol_lang_itm(Xoa_lang_mgr lang_mgr, byte[] key_bry) {
		...
		Xol_lang_stub lang_itm = Xol_lang_stub_.Get_by_key_or_null(key_bry);
                if (lang_itm == null)
                    lang_itm = Xol_lang_stub_.Get_by_key_or_null(Bry_.new_a7("en")); // always find something !! 20211209
		...
		this.case_mgr2 = DB_case_mgr_.U8();
	...
	public DB_case_mgr			Case_mgr2() {return case_mgr2;} private DB_case_mgr case_mgr2;
	...
	public Xol_lang_itm Init_by_load() {
		synchronized (thread_lock) { // Scribunto can create langs outside of wiki_lang; EX:dewiki and multiple scripts call isRTL for fr; ISSUE#:330; DATE:2019-02-09
			if (!loaded) {
				...
				msg_mgr.Itm_by_key_or_new(Bry_.new_a7("Lang")).Atrs_set(key_bry, false, false, false);	// set "Lang" keyword; EX: for "fr", "{{int:Lang}}" -> "fr"
				Load_lang(key_bry);

...
function drawLegend (ctx, data) {
	...
	if (data.legend.position === 'bottom' && cols == 1) {
		entriesPerCol = Math.ceil((data.canvas.bottom - data.legend.top)/17);
	}
...
//	ctx.lineCap = 'round'; //For some strange reason, Firefox treats 'butt' and 'square' as 'square', and 'round' as 'butt'. We need 'butt', so set it to 'round'.
	...
	ctx.strokeStyle = '#4545ed';
	ctx.moveTo(260, 0);
	ctx.lineTo(260, data.canvas.top);
	ctx.closePath();
	ctx.stroke();

	ctx.beginPath();
	ctx.strokeStyle = '#4545ed';
	ctx.moveTo(0, data.canvas.top);
	ctx.lineTo(260, data.canvas.top);
	ctx.closePath();
	ctx.stroke();

...
if (window.xowa_popups_gotpopup == null) {
  window.xowa_popups_gotpopup = xowa_popups_gotpopup;
}
...
var namespace_pattern = /(File:|Category:|Talk:|User:)/;
var extension_pattern = /(\.svg|\.ogv|\.jpg|\.png|\.ogg)$/;
function show_init(elem, popup_itm, ev) {
  ...
  if (!popup_itm.html || popup_itm.href === '/wiki/Special:XowaPopupHistory') {
    xowa_exec('popups_get_html', popup_itm, ev, '');
    ...
    //popup_itm.html = html;
	  ...
	  return xowa_popups_gotpopup(popup_itm, ev);
}

function xowa_popups_gotpopup(popup_itm, ev)
{
	if (!popup_itm.html)
	{
		var elem = popup_itm.anchor;
		elem.attr('title', elem.data('title')); // restore tooltip
		return;
	}
  ...
  */
  ...
  /*
var ret =  show_popup(popup_itm, $popup, ev.clientX, ev.clientY, window_max_w);
return ret;
*/
  // return 
  var ret = show_popup(popup_itm, $popup, ev.clientX, ev.clientY, window_max_w)
  popup_itm.anchor.data().popupRef = $popup;
  $popup.hover(
					function () {
						show(popup_itm.anchor, ev);
					}, function () {
						hide(popup_itm.anchor);
					});
}
...
function show ($this, ev) {
  ...
  if (/#/.test(href)) return;   // ignore any "#" 
  if (namespace_pattern.test(href)) return;
  if (protocol_pattern.test(href)) return;  // ignore hrefs with absolute protocol of "http:", etc. which won't point to XOWA content
  if (extension_pattern.test(href)) return // ignore images
  ...
  if (!$this.data('title')) {
	...
	$this.data({
		...
		showTimer: window.setTimeout(function () {
			var xx = show_init($this, popup_itm, ev);
			$this.data({
				willShow: false,
				popupVisible: true,
        popup_itm: popup_itm,
...
function show_popup(popup_itm, $popup, anchor_x, anchor_y, popup_max_w) {
  ...
  if (window.xowa.js.mathJax == null)
    ...
    window.xowa.js.load_lib(xowa.root_dir + 'bin/any/xowa/xtns/Math/modules/mathjax/xowa_mathjax.js', xowa_mathjax_run); // note that this will only load mathjax if math items are on page
  else
    xowa_mathjax_run();
...
function hide ($this) {
	...
	$this.data({
		...
		hideTimer: window.setTimeout(function () {
			...
			reallyHide(data.popup_itm);
		}, cfg.hide_delay)
	});
}

function reallyHide (popup_itm) {
  var $popup = $("#" + popup_itm.id);

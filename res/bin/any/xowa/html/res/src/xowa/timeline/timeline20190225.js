...
function getCanvasSize (data) {
	...
	if (data.legend.show) {
		...
		if (parseLength(data.legend.top) == 0) {
...
function validateLegend(data) {
	...
	if (legend.position === 'top') {
		...
		if (legend.top === undefined) {
			legend.top =
				(data.canvas.bottom + data.canvas.height - 20);
		}
	}
	return legend;
}
...
function drawBar (ctx, data, bar) {
	...
	//if ((bar.mark && bar.mark !== 'none') || x1 - x0 == 0 || y1 - y0 == 0){
...
//	v0 = data.barPositions[bar.bar].pos;
	...
	if (bar.mark && bar.mark !== 'none') {
		...
		console.log('mark ' + bar.text + '- ' + (data.xAxisIsTime ? x1 : x0 - 10) + ',' + (data.xAxisIsTime ? y0 : y1) + ',' + (data.xAxisIsTime ? x1 : x0 + 10) + ',' + y1 + ':' + pos.x + ',' + pos.y);
		//ctx.lineTo(x1, y1);
		ctx.closePath();
		ctx.stroke();
	}

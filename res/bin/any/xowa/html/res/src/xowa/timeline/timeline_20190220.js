...
function parseShift(shift) {
	var match, pos ={x:0,y:0};
	match = (/^\(\s*([0-9.\-]*)\s*,\s*([0-9.\-]*)\s*\)$/).exec(shift);
	if (match) {
		pos.x = parseLength(match[1]);
		pos.y = parseLength(match[2]);
	} else {
		match = (/^\(\s*([0-9.\-]*)\s*\)$/).exec(shift);
		if (match) {
			pos.x = parseLength(match[1]);
		}
	}
...
function parsePlotData (data, lastPlotData) {
	...
	if (data.at || data.from || data.till) {
		lastPlotData.nobar = false;
/*		// extra work for shift - need to merge
		if (data.shift && lastPlotData.globalshift) {
			var datapos = parseShift(data.shift);
			var lastpos = parseShift(lastPlotData.globalshift);
			data.shift = '(' + String(datapos.x + lastpos.x) + ',' + String(datapos.y + lastpos.y) + ')'
		}
*/
		// if no local shift and we have a global shift use it
		if (data.shift == undefined && lastPlotData.globalshift) {
			data.shift = lastPlotData.globalshift;
		}
		...
		if (data.shift && lastPlotData.nobar) {// global shift?
			lastPlotData.globalshift = data.shift;
			lastPlotData.nobar = false;
		}
		return false;
	}
...
function parseCode (code) {
	var	commands = splitIntoCommands(replaceDefinesAndPresets(removeCommentsAndEmptyLines(code))),
		ret = {
			...
			extra: {legendColors: [], columns:0, percol:0 }
		}, data, i, j,
		lastLineData, lastPlotData, lastTextData;
	for (i = 0; i < commands.length; i++) {
		lastLineData = {width: '2', color: 'black'};
		lastPlotData = {bar: '', anchor: 'middle', color: 'rgb(0,0.6,0)', fontsize: 'S', nobar:true, width: '25'}; //documentation says width is calculated if missing, but I don't see this
...
function getCanvasSize (data) {
	...
	// need to calc number of legend columns in case of 'auto' height
	if (data.legend.show) {
		var legend = [], cols, color, entriesPerCol;
		...
		data.extra.columns = cols;
		data.extra.legendColors = legend;
		data.extra.percol = entriesPerCol;
	}
	...
	if (data.imageSize.height === 'auto') {
		...
		if (data.legend.show) {
			var legendsize = data.extra.percol * 17;
			console.log((ret.bottom-40)/17);
			if (ret.bottom < legendsize + 40) {
				data.extra.percol = parseInt((ret.bottom-40)/17);
			}
//			ret.height += legendsize - ret.bottom;
//			ret.bottom = legendsize;
		}
	} else {
		...
		if (data.plotArea.height) {
		  ...
		  console.log("left:"+ret.left+" right:"+ret.right+" width:"+ret.width);
		  console.log("top:"+ret.top+" bottom:"+ret.bottom+" height:"+ret.height);
		  console.log("innerHeight:"+ret.innerHeight+" innerWidth:"+ret.innerWidth);
		  console.log("e "+data.extra.percol);
		  console.log("p "+(data.extra.percol * 17 + 5));
	return ret;
}
...
function drawTextAtPos (ctx, text, x, y, options) {
	...
	while (true) {
		...
		if (linked) {
			...
			ctx.strokeStyle = '#0a0adb' //'#4545ed'; ??'#3931ac'
			ctx.fillStyle = '#0a0adb' //'#4545ed';
...
function drawMultiTabText (ctx, text, options) {
	...
	for (i = 0; i < tabChunks.length; i++) {
		...
		rawText = tabChunks[i];
		// extra check for quoted string
		var a=rawText.length;
...
function drawMultiLineText (ctx, data, text, pos) {
	...
	//ctx.font = '700 ' + fontsize + 'px Code New Roman';
	//ctx.font = fontsize + 'px Code New Roman';
	ctx.font = 'bold ' + fontsize + 'px sans-serif';
	//ctx.font = 'bold ' + fontsize + 'px "Vast Shadow"';
	...
	if (bar.text) {
		...
		if (bar.shift) {
			...
			if (match) {
				...
				match = (/^\(\s*([0-9.\-]*)\s*\)$/).exec(bar.shift);
				if (match) {
					pos.x += parseLength(match[1], data.canvas.width);
				}
			}
		}
		pos.y += 2; // ?????? to make it work!
...
function drawLegend (ctx, data) {
	...
	cols = data.extra.columns;
	legend = data.extra.legendColors;
	entriesPerCol = data.extra.percol;
/*	for (color in data.colors) {
		if (!data.colors[color].legend || data.colors[color].legend === 'n' || data.colors[color].legend === 'N') {
			continue;
		}
	...
	if (data.legend.orientation === 'hor') {
		...
		x = parseLength(data.legend.left, data.canvas.width) || (data.legend.position === 'right' ? data.canvas.width - data.canvas.right + 5 : data.canvas.left);
	  ...
	  console.log("y " + y + "ch " + data.canvas.height);
	  console.log(17 * entriesPerCol + 5);
	}
	oldY = y;
	textWidthLargest = 0;
	for (i = 0; i < legend.length; i++) {
		...
		if (data.legend.orientation === 'hor') {
			...
			if ((i + 1) % entriesPerCol) {
				...
				x += Math.round(textWidthLargest * 1.2) + 26;
				y = oldY;
				textWidthLargest = 0;
			}
		}
...
//	ctx.lineCap = 'round'; //For some strange reason, Firefox treats 'butt' and 'square' as 'square', and 'round' as 'butt'. We need 'butt', so set it to 'round'.
	ctx.lineCap = 'butt'; //For some strange reason, Firefox treats 'butt' and 'square' as 'square', and 'round' as 'butt'. We need 'butt', so set it to 'round'.
	drawAxes(ctx, data);
	drawLegend(ctx, data);

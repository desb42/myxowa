...
( function () {
	...
	function buildDateTable() {
		...
		for ( i = 0; i < 12; i++ ) {
			name = xowa_global_values.wgMonthNames[ i+1 ].toLowerCase();
			ts.monthNames[ name ] = i + 1;
			regex.push( mw.RegExp.escape( name ) );
			name = xowa_global_values.wgMonthNamesShort[ i+1 ].toLowerCase().replace( '.', '' );
			ts.monthNames[ name ] = i + 1;
			regex.push( mw.RegExp.escape( name ) );
		}
...
/* ------ add from mediawiki/resources/src/mediawiki.RegExp.js --- */
( function () {
	/**
	 * @class mw.RegExp
	 */

...
function get_optlist() {
	...
	if ( !optlist ) {
		...
		if (mw.user.options.get( 'skin' ) == 'vector' ) displayOptions.className = "portal collapsed";
			else displayOptions.className = "portlet";
		if (mw.user.options.get( 'skin' ) == 'vector' ) cl = "body";
			else cl = "pBody";
...
function pr_typos() {
	...
	if (spanlist.length) {
		...
		mw.util.addPortletLink ('p-displayOptions', '<a href="javascript:pr_popup();">', ws_msg('corr_link')+" ("+spanlist.length+")", 'option-corrections', '' );
